...
import { GLOBAL_CONFIG, getBaseUrl } from '../global-config.js';
...
(async () => {
  ...
  const base = process.argv[2] || process.env.BASE_URL || getBaseUrl();
  const urls = GLOBAL_CONFIG.validators.navigation.testUrls.map(path => `${base}${path}`);
  ...
  for (const url of urls) {
    try {
      ...
      const resp = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: GLOBAL_CONFIG.build.timeouts.navigation });
