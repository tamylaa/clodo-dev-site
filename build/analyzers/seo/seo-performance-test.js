...
import config from '../clodo.config.js';
...
class SEOTester {
    ...
    async testSitemap() {
        ...
        try {
            const sitemapPath = path.join(__dirname, '..', config.build.publicDir, 'sitemap.xml');
    ...
    async fetchPage(path, returnRaw = false) {
        ...
        const fullPath = path.join(__dirname, '..', config.build.publicDir, filePath);
